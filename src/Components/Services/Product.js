...
import Validation from   "./Vadilation";
export default class Product extends ProductsApi{
    save = async ()=>{
        ...
        if(productType.value!=="0"){
            ...
            basicInps.forEach(item =>product[item.id] = item.value);
            ...
            if(!Validation.validSKU(product.sku)){
                err+="<li>Please, provide a valid SKU with length of 8 symbols. Uppercase letters and numbers allowed</li>";
            }
            if(!Validation.validName(product.name)){
                err+="<li>Please, provide a valid name with min length of 3 symbols. Letters and numbers allowed</li>";
            }
                ...
                if(answer.ok){
                    if(answer.data.errors){
                        ...
                        for(let errorName in errors){
                            err+=`<li>${errors[errorName]}</li>`;
                        }
                        errorsWrapper.innerHTML = err;
                    }
                    else return await answer.ok;
                }
            }
